...
class SettingsViewController: UIViewController {
    ...
    private lazy var termsButton: UIButton = {
        let button = UIButton()
        button.setTitle(NSLocalizedString("Terms and Conditions", comment: ""), for: .normal)
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            termsButton.topAnchor.constraint(equalTo: notificationsLabel.bottomAnchor, constant: 40),
    ...
    private func showNotificationAlert() {
        let alert = UIAlertController(title: NSLocalizedString("Error", comment: "Title for error alert"),
                                      ...
                                      preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Button to acknowledge error"), style: .default, handler: nil))
        present(alert, animated: true, completion: nil)
    }
}
