...
class SwipeCardViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    private var advice: UILabel = {
        let advice = UILabel()
        advice.font = .boldSystemFont(ofSize: 20)
        advice.textAlignment = .center
        advice.textColor = .black
    ...
    override func viewDidLoad() {
        ...
        cardViewControllers = cardViews.map { cardView in
            ...
            for cardView in cardViews {
                ...
                if cardView.card.word == "MAFIA" || cardView.card.word == "МАФИЯ"{
    ...
    private func addConstraints(to viewController: UIViewController) {
        ...
        viewController.view.addSubview(advice)
        ...
        NSLayoutConstraint.activate([
            ...
            advice.bottomAnchor.constraint(equalTo: viewController.view.bottomAnchor, constant: 1),
            advice.centerXAnchor.constraint(equalTo: viewController.view.centerXAnchor)
        ])
    }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, willTransitionTo pendingViewControllers: [UIViewController]) {
        if let nextViewController = pendingViewControllers.first,
           let targetIndex = cardViewControllers.firstIndex(of: nextViewController) {
            ...
            advice.isHidden = targetIndex == cardViewControllers.count - 1
        }
    }
